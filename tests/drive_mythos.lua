...
--
--   POKEPORT_DRIVER=mods/mythos/tests/drive_mythos.lua xvfb-run -a love .
--
...
--
-- It prints MYTHOS: lines and quits with a verdict. It captures no images:
...
return function()
  ...
  local function report(ok, label, detail)
    ...
    print(("MYTHOS: %-4s %s%s"):format(ok and "ok" or "FAIL", label,
  ...
  for _, m in ipairs((Game.mods and Game.mods.mods) or {}) do
    if m.id == "mythos" then loaded = m.enabled and true or false end
  end
  if type(Game.mods.mods) == "table" and not loaded then
    for id, m in pairs(Game.mods.mods) do
      if id == "mythos" or (type(m) == "table" and m.id == "mythos") then
        loaded = (m.enabled ~= false)
      end
    end
  end
  report(loaded, "the mythos mod is loaded and enabled",
  ...
  do
    ...
    report(where == "MYTHOS_BILLS_GARDEN",
      "walking into the corner opens the garden", "landed on " .. tostring(where))

    if where == "MYTHOS_BILLS_GARDEN" then
      ...
      report(savedMap ~= "MYTHOS_BILLS_GARDEN",
  ...
  print(("MYTHOS: VERDICT %s  (%d checks, %d failed)")
