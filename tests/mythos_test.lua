-- Standalone: luajit mods/mythos/tests/mythos_test.lua
--
...
local ID = "mythos"
local DIR = os.getenv("MYTHOS_DIR") or "mods/mythos"
...
-- pcalls it, so a save pointing at MYTHOS_BILLS_GARDEN with this mod
...
if KANTO then
  T.check(Data.sprites.MYTHOS_SPRITE_TRUCK ~= nil, "the truck sprite is registered")
  local truck
  for _, o in ipairs(Data.maps.VERMILION_DOCK.objects or {}) do
    if o.sprite == "MYTHOS_SPRITE_TRUCK" then truck = o end
  end
...
do
  local path = Data.sprites.MYTHOS_SPRITE_TRUCK.image
  T.check(type(path) == "string" and path:find("mythos", 1, true),
    "the truck's art comes from this mod, not from the ROM cache")
  T.check(Data.sprites.MYTHOS_SPRITE_TRUCK.source == nil,
